...
from models.vmunet_r3.vmamba.csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1, getCSM
from models.vmunet_r3.vmamba.csm_triton import CrossScanTritonF, CrossMergeTritonF, CrossScanTriton1b1F
from models.vmunet_r3.vmamba.csms6s import CrossScan, CrossMerge
from models.vmunet_r3.vmamba.csms6s import CrossScan_Ab_1direction, CrossMerge_Ab_1direction, CrossScan_Ab_2direction, CrossMerge_Ab_2direction
from models.vmunet_r3.vmamba.csms6s import SelectiveScanMamba, SelectiveScanCore, SelectiveScanOflex
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        self.up_3_2 = nn.Sequential(
            PatchExpand2D(dim=768, dim_scale=2),
            PatchExpand2D(dim=384, dim_scale=2)
        )

        self.up_2_1 = nn.Sequential(
            PatchExpand2D(dim=384, dim_scale=2),
            PatchExpand2D(dim=192, dim_scale=2)
        )
    ...
    def forward_features(self, x):
        ...
        x1 = self.pos_drop(x) # (96, 56, 56)

        x2 = self.layers[0](x1)  # (192, 28, 28)

        x3 = self.layers[1](x2) # (384, 14, 14)

        x4 = self.layers[2](x3) # (768, 7, 7)
    ...
    def forward(self, x):
        y1 = self.forward_features(x)
        y = self.forward_final(y1)
