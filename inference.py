...
def get_model(ckpt: str, model_name: str, **kwargs: Any) -> nn.Module:
...
def test_acdc(ckpt: str, model_name: str) -> None:
    ...
    logger.add(os.path.join(output_folder, model_name, "testing.log"))

    device = "cuda:0"
    model = get_model(ckpt=ckpt, model_name=model_name, in_channels=3, num_classes=4).to(device)
    print(device)
...
def test_synapse(ckpt: str, model_name: str) -> None:
    ...
    device = "cuda:0"
    model = get_model(ckpt=ckpt, model_name=model_name, in_channels=3, num_classes=9).to(device)
...
if __name__ == '__main__':
    ...
    if dataset == "synapse":
        test_synapse(ckpt=ckpt, model_name=model_name)
    else:
        test_acdc(ckpt=ckpt, model_name=model_name)
